	...
	while(LinkStack_Size(stack))
	{
		...
		for(i = 0; i < LinkList_Length(gra->la[v]); i++)
		{
			...
			if( visited[node->v] == 0 )
			{
				...
				//把剩余的保存起来 
				for(j = (LinkList_Length(gra->la[v]) - 1) ; j > i; j--)
				{
					node = (LGraphLinkListNode*)LinkList_Get(gra->la[v], j);
					
					LinkStack_Push(stack, gra->v + node->v);
				}
		...
		if(flag == 0 && LinkStack_Size(stack))
		{
			for(i = 0; i < LinkStack_Size(stack); i++)
			{
				int v1 = (LVertex**)LinkStack_Pop(stack) - (gra->v);
